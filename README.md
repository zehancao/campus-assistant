...
**schema** 见 [docs/schema.sql](docs/schema.sql)

### 当前版本：v2.0（2026-07-11 更新）

#### 主要变更

| 改动项 | 说明 |
|--------|------|
| 🆕 `classes` 班级表 | 学生所属班级，支持按班级匹配课表 |
| 🆕 `teachers` 教师表 | 独立教师实体，含工号/职称/学院 |
| 🔄 课表体系重构 | `courses`(课程基础) → `course_schedules`(上课安排) → `course_class`(班级关联) |
| 🆕 `student_timetable` 缓存表 | 学生端一键同步课表，直接查缓存无需关联计算 |
| 🆕 `week_parity` 字段 | 支持单双周交替（all/odd/even），解决中飞院单双周排课 |
| 🆕 `products.tags` 字段 | 商品自定义标签 JSON，提升搜索粒度 |
| 🆕 `notifications.push_status` 字段 | 推送状态追踪（未推/已推/已送达/已读） |
| 📈 索引优化 | 全表补充高频查询组合索引 |
